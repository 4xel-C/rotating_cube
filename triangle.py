...
A, B, C = 0.0, 0.0, 0.0
cubeWidth = 20
width, height = 200, 44  # screen size in character number
zBuffer = (
    [0] * (width * height)
)  # will keep track of the Z value of each point to determine which point to display in 2D
buffer = [" "] * (
    width * height
)  # will recieve each character to display to the screen

# z offset to not put the camera in the center of the cube
distanceFromCam = 100
...
def calculateForSurface(cubeX, cubeY, cubeZ, ch):
    ...
    z = (
        calculateZ(cubeX, cubeY, cubeZ) + distanceFromCam
    )  # apply an offset for Z not to have the camera on origin
    ...
    # Calculate the position in the character array (pixel position) and center the cube
    # K1 => focal length
